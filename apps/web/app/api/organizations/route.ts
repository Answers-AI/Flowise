...
export async function PATCH(req: Request) {
    try {
        ...
        if (!session?.user?.email || session?.user?.organizationId !== id) return NextResponse.redirect('/auth')
        ...
        // @ts-ignore-next-line
        const dbFields = prisma._runtimeDataModel.models.Organization.fields.map((field: any) => field.name)
